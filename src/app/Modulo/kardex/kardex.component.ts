...
/*productoForm = new UntypedFormGroup({
  ...
  movimientoForm = new UntypedFormGroup({
    ...
    cantidad: new FormControl('', [Validators.required, Validators.min(1), Validators.max(1000)]),
...
seleccionarProducto(producto: any, input: HTMLInputElement) {
  ...
  this.buscarProducto = `${producto.codigo} - ${producto.nombre}`;
  this.mostrarListaProductos = false;
  input.blur();
}
...
validarFormulario(): boolean {
  ...
  if (this.movimientoForm.get('cantidad')?.invalid) return false;
  if (this.movimientoForm.get('tipo_movimiento')?.invalid) return false;

  const cantidad = this.movimientoForm.get('cantidad')?.value;
  if (cantidad <= 0 || cantidad > 1000) return false;
